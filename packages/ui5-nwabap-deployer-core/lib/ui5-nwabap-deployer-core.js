 ...
 * @param {Object} oOptions Optons
 * @param {Object} oLogger Logger
 */
function uploadWithTransportUserMatch(oTransportManager, oOptions, oLogger) {
    oTransportManager.determineExistingTransport(function (oError, sTransportNo) {
        if (sTransportNo) {
            oFileStoreOptions.ui5.transportno = sTransportNo;
            syncFiles(oFileStoreOptions, oLogger, oOptions.resources.fileSourcePath, aFiles);
            ...
            oLogger.error(new Error('No transport found and create transport was disabled!'));
        }
    });
}
 ...
 * @param {Object} oLogger Logger
 */
exports.deployUI5toNWABAP = function (oOptions, aFiles, oLogger) {
    var oAdaptedOptions = {};
    ...
    if (oAdaptedOptions.ui5.package !== '$TMP' && oAdaptedOptions.ui5.transportno === undefined) {
        ...
        if (oAdaptedOptions.ui5.transport_use_user_match) {
            uploadWithTransportUserMatch(oTransportManager, oAdaptedOptions);
            ...
            oTransportManager.createTransport(oAdaptedOptions.ui5.package, oAdaptedOptions.ui5.transport_text, function (oError, sTransportNo) {
                ...
                oFileStoreOptions.ui5.transportno = sTransportNo;
                syncFiles(oFileStoreOptions, oLogger, oAdaptedOptions.resources.sFileSourcePath, aFiles);
            });
        } else {
            oLogger.error(new Error('No transport configured but create transport and user match was disabled!'));
            return;
        }
    } else {
        syncFiles(oFileStoreOptions, oLogger, oAdaptedOptions.resources.sFileSourcePath, aFiles);
